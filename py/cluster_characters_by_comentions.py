...
parser.add_argument('-i', '--narrator_ishmael', help='Regard Ishmael as being mentioned in every chapter.', default=False, action='store_true')
...
def plotCharacterComentionsSave(total_mentions, chapter_mentions, character_list):
    """
    ...
    Arguments:  total_mentions, the total number of chapters in which each character is mentioned
                chapter_mentions, matrix of character co-mentions
    ...
    plotChapterCoMentions(sorted_chapter_mentions, sorted_character_list, title='Normed chracter co-mentions', figsize=(10,8))
...
if args.is_normed:
    chapter_mentions, total_mentions = getNormedCharacterCoMentions(character_list, num_to_chap_title, moby_dick_text, character_list_with_doubles, args.narrator_ishmael)
else:
    chapter_mentions, total_mentions = getCharacterCoMentions(character_list, num_to_chap_title, moby_dick_text, character_list_with_doubles, args.narrator_ishmael)
chapter_mentions = nnr.checkDirected(chapter_mentions.copy())
chapter_mentions_comp, keep_indices, comp_assign, comp_size = nnr.getBiggestComponent(chapter_mentions.copy())
null_samples_eig_vals, optional_returns = nnr.getPoissonWeightedConfModel(chapter_mentions_comp, 100, return_eig_vecs=True, is_sparse=True)
...
network_modularity_matrix = chapter_mentions_comp - expected_sparse_wcm
...
signal_chapter_mentions = chapter_mentions_comp[reject_dict['signal_inds']][:, reject_dict['signal_inds']]
if reject_dict['signal_inds'].size == 0:
    ...
    biggest_signal_chapter_mentions_comp, biggest_signal_chapter_mentions_comp_inds, biggest_signal_chapter_mentions_comp_assign, biggest_signal_chapter_mentions_comp_size = nnr.getBiggestComponent(signal_chapter_mentions)
    biggest_signal_chapter_mentions_comp_inds = reject_dict['signal_inds'][biggest_signal_chapter_mentions_comp_inds]
    degree_distn, leaf_inds, keep_inds = getLeavesKeeps(biggest_signal_chapter_mentions_comp)
    final_chapter_mentions_inds = biggest_signal_chapter_mentions_comp_inds[keep_inds]
    signal_chapter_mentions_leaf_inds = biggest_signal_chapter_mentions_comp_inds[leaf_inds]
    final_chapter_mentions = biggest_signal_chapter_mentions_comp[keep_inds][:, keep_inds]
    final_character_list = np.array(character_list)[final_chapter_mentions_inds]
    leaf_characters = np.array(character_list)[signal_chapter_mentions_leaf_inds]
    ...
    signal_expected_wcm = expected_sparse_wcm[final_chapter_mentions_inds][:, final_chapter_mentions_inds]
    max_mod_cluster, max_modularity, consensus_clustering, consensus_modularity, consensus_iterations, is_converged = nnr.consensusCommunityDetect(final_chapter_mentions, signal_expected_wcm, exceeding_space_dims+1, exceeding_space_dims+1)

# saved_name = plotCharacterComentionsSave(total_mentions, chapter_mentions, character_list)
